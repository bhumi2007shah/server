...
public interface JobCapabilitiesRepository extends JpaRepository<JobCapabilities, Long> {
    ...
    @Modifying
    @Query(value = "UPDATE JobCapabilities jb set SELECTED =:boolValue where jb.JOB_ID =:jobId", nativeQuery=true)
    void updateJobCapabilitiesForUnSelected(@Param("boolValue") Boolean boolValue, @Param("jobId") Long jobId);

    @Modifying
    @Query(value = "UPDATE JobCapabilities jb set SELECTED =:boolValue where jb.JOB_ID =:jobId and jb.ID IN :jobCapabilitiesIdList", nativeQuery=true)
